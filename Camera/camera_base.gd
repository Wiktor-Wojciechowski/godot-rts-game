...
@export_range(0, 100, 1) var camera_zoom_min:float = 5.0 #cant be 0
@export_range(0, 100, 1) var camera_zoom_max:float = 40.0 
...
@export_range(0,20,1) var camera_base_rotation_speed:float = 10.0 
@export_range(0, 10, 1) var camera_socket_rotation_x_min:float = -1.5
@export_range(0, 10, 1) var camera_socket_rotation_x_max:float = 0

var default_camera_rotation_y:float = 0
var default_camera_rotation_x:float = 0
...
#camera bounds
@export var min_camera_bounds = Vector3(-10, 0, -10)
@export var max_camera_bounds = Vector3(10, 0, 10)
...
func _ready() -> void:
	default_camera_rotation_y = rotation.y
	default_camera_rotation_x = camera_socket.rotation.x


# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta:float) -> void:
	global_position.x = clamp(global_position.x, min_camera_bounds.x, max_camera_bounds.x)
	global_position.z = clamp(global_position.z, min_camera_bounds.z, max_camera_bounds.z)
...
func _unhandled_input(event:InputEvent) -> void:
	...
	if(event.is_action_pressed("reset_camera_rotation")):
		camera_socket.rotation.x = default_camera_rotation_x
		rotation.y = default_camera_rotation_y
...
func camera_socket_rotate_x(delta:float, dir:float) -> void:
	...
	new_rotation_x -= dir * camera_rotation_speed * delta
...
func camera_automatic_pan(delta:float) -> void:
	...
	if ((current_mouse_position.x < margin) or (current_mouse_position.x > viewport_size.x - margin)):
		if current_mouse_position.x > viewport_size.x/2:
			pan_direction.x = 1
		translate(Vector3(pan_direction.x * camera_automatic_pan_speed * delta * zoom_factor, 0, 0))
	...
	if ((current_mouse_position.y < margin) or (current_mouse_position.y > viewport_size.y - margin)):
		if current_mouse_position.y > viewport_size.y/2:
			pan_direction.y = 1
		translate(Vector3(0, 0, pan_direction.y * camera_automatic_pan_speed * delta * zoom_factor))

func get_Vector2_from_Vector3(unproject_from_vector3:Vector3) -> Vector2:
	return camera.unproject_position(unproject_from_vector3)
